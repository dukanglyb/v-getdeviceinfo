...
const MethodLibrary = (() => {
	return {
		...
		getDate: function () {
				...
				return `${year}/${datePad(month)}/${datePad(day)} ${datePad(hour)}:${datePad(minutes)}:${datePad(seconds)}`
		},
		// 获取匹配库
		...
		getBrowserInfo: function () {
				...
				if (_this.browser == 'Chrome' && parseInt(_this.browserVersion) > 27) {
						_this.engine = 'Blink'
				} else if (match['Chrome'] && _this.engine == 'WebKit') {
						_this.engine = 'Blink';
 ...
 * @return {*}
 * @author: liyb-d
 * @Date: 2021-08-06 11:22:15 +0800
 */
export const LogicLibrary = (() => {
	return {
 ...
 * @return {*}
 * @author: liyb-d
 * @Date: 2021-08-06 11:25:03 +0800
 */
export const getDeviceInfo = (params, callback) => {
